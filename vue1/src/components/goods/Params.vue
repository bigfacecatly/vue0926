  ...
  <div>
    ...
    <el-dialog :title="'添加' + titleText" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addRarams">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 修改参数的对话框 -->
    <el-dialog :title="'修改' + titleText" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editRarams">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // tab 页签点击事件的处理函数
    handleTabClick () {
      console.log(this.activeName, '页签点击事件的处理函数')
      this.getParamsData()
    },
    // 获取参数的列表数据
    async getParamsData () {
    ...
    async handleInputConfirm (row) {
      console.log('ok', row.inputVisible)
      if (row.inputValue.trim().length === 0) {
        row.inputValue = ''
        row.inputVisible = false
        return false
      }
      // 如果没有return,则证明了输入正确的内容，需要后续处理
    ...
    showInput (row) {
      ...
      console.log(1111, row.inputVisible)
      this.$nextTick(_ => {
